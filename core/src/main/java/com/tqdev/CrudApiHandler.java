...
import java.util.Map; // Added import for java.util
...
public class CrudApiHandler extends AbstractHandler {
    ...
    public void handle(String target, Request baseReq, HttpServletRequest req, HttpServletResponse resp) 
        ...
        String[] columns = input==null?(new String[0]):(String[])input.keySet().toArray(new String[0]);
        ...
        try {
            ...
            for (int i=0;i<columns.length;i++) {
                statement.setObject(i+1, input.get(columns[i]));
            }
